...
tmp_folder = Path(__file__).parent / f"{Path(__file__).stem}_config"
os.makedirs(tmp_folder, exist_ok=True)
log_folder = Path(__file__).parent / f"{Path(__file__).stem}_config" / "log"
os.makedirs(log_folder, exist_ok=True)



CONFIG_FILE = tmp_folder / "config.json"
EXPORT_FILE = tmp_folder / "depth_export.xml"
BLENDER_FILE = tmp_folder / "blender_script.py"
TURNTABLE_FILE = tmp_folder / "turntable_scene.blend"
DATA_FILE = Path(__file__).parent / "data_output.csv"
MASK_FILE = tmp_folder / "maskReconReg.rsbox"
TEMP_MASK = tmp_folder / "temp_mask.png"


RENDER_SCENE = tmp_folder / "turntable_scene.blend"
RENDER_SCRIPT = tmp_folder / "turntable_render.py"

default_rs_location = r"C:\Program Files\Epic Games\RealityScan_2.0\RealityScan.exe"
default_bl_location = r'C:\Program Files\Blender Foundation\Blender 4.5\blender.exe'
...
class MainWindow(QWidget):
    ...
    def export_ctrlPts(self):
        ...
        cmd = [
            ...
            "-headless", "-reset", "cfg", "-stdConsole",
            "-silent", "c:\\CrashReportFolder",
            "-load", fr"{folder}\{nodule_name}.rsproj",
            "-set", '"appProcessActionTime=0"',
            "-exportControlPointsMeasurements", controlPts_csv,
            "-quit"
        ]
    ...
    def align_next_loop(self):
        ...
        if Path(controlPts_csv).exists():
            ...
            cmd = [
                ...
                "-addFolder", folder,
                "-align",
                "-importControlPointsMeasurements", LOOP_CSV,
                "-defineDistance", f'{controlPt_1}', f'{controlPt_2}', '0.01',
                # "-generateAIMasks",
                "-selectControlPoint", f'{controlPt_1}',
                ...
                "-update",
                "-setReconstructionRegion", MASK_FILE,
                "-moveReconstructionRegion", "0", "0", f'{mask_offset}',
    ...
    def export_mesh(self):
        ...
        cmd = [self.rs_exe,
            ...
            *add_folder_args,
            ...
            "-calculateTexture",
            "-save", PROJ_FILE_models,
            "-exportModel", "Model 1", fr"{folder}\{nodule_name}_mesh.obj",
            "-quit"]
